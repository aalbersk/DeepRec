...
USER_COLUMN = [
    '101', '109_14', '110_14', '127_14', '150_14', '121',
...
TAG_COLUMN = ['109_14', '110_14', '127_14', '150_14', '210']
NOT_USED_CATEGORY = ['301']

USED_FEATURE_COLUMNS = USER_COLUMN + ITEM_COLUMN + COMBO_COLUMN
ALL_FEATURE_COLUMNS = USED_FEATURE_COLUMNS + NOT_USED_CATEGORY
ALL_INPUT = LABEL_COLUMNS + ALL_FEATURE_COLUMNS

HASH_BUCKET_SIZES = {
    '101': 100000, # User ID
    '109_14': 100000, # User historical behaviors of category ID and count
    '110_14': 100000, # User historical behaviors of shop ID and count
    '127_14': 100000, # User historical behaviors of brand ID and count
    '150_14': 100000, # User historical behaviors of intention node ID and count
    '121': 10000, # Categorical ID of User Profile
    '122': 10000, # Categorical group ID of User Profile
    '124': 10, # Users Gender ID
    '125': 10, # Users Age ID
    '126': 10, # Users Consumption Level Type I
    '127': 10, # Users Consumption Level Type II
    '128': 10, # Users Occupation: whether or not to work
    '129': 100, # Users Geography Informations
    '205': 100000, # Item ID
    '206': 10000, # Category ID to which the item belongs to
    '207': 10000, # Shop ID to which item belongs to
    '210': 100000, # Intention node ID which the item belongs to
    '216': 10000, # Brand ID of the item
    '508': 100000, # The combination of features with 109_14 and 206
    '509': 100000, # The combination of features with 110_14 and 207
    '702': 100000, # The combination of features with 127_14 and 216
    '853': 100000 # The combination of features with 150_14 and 210
}
...
def generate_input_data(filename, batch_size, num_epochs, seed):
    def parse_csv(value):
        ...
        string_defaults = [[' '] for i in range(0, len(ALL_FEATURE_COLUMNS))]
        label_defaults = [[0] for i in range (0, len(LABEL_COLUMNS))]
        column_headers = ALL_INPUT
...
def build_feature_cols():
    ...
    for column_name in ALL_FEATURE_COLUMNS:
        if column_name in NOT_USED_CATEGORY:
            continue
        if column_name in USED_FEATURE_COLUMNS:
            ...
            raise ValueError(f'Unexpected column name {column_name} occured.')
...
class ESMM():
    ...
    def __build_model(self):
        ...
        with tf.variable_scope('ESMM', reuse=tf.AUTO_REUSE):
            ...
            user_in_scope = tf.variable_scope('user_mlp_layer',
                                              partitioner=self.__dense_layer_partitioner,
                                              reuse=tf.AUTO_REUSE)
            with user_in_scope.keep_weights() if self.__bf16 else user_in_scope:
            ...
            item_in_scope = tf.variable_scope('item_mlp_layer',
                                              partitioner=self.__dense_layer_partitioner,
                                              reuse=tf.AUTO_REUSE)
            with item_in_scope.keep_weights() if self.__bf16 else item_in_scope:
            ...
            combo_in_scope = tf.variable_scope('combo_mlp_layer',
                                               partitioner=self.__dense_layer_partitioner,
                                               reuse=tf.AUTO_REUSE)
            with combo_in_scope.keep_weights() if self.__bf16 else combo_in_scope:
    ...
    def __build_cvr_model(self, net):
        cvr_scope = tf.variable_scope('cvr_mlp',
                                      partitioner=self.__dense_layer_partitioner,
                                      reuse=tf.AUTO_REUSE)
        with cvr_scope.keep_weights() if self.__bf16 else cvr_scope:
    ...
    def __build_ctr_model(self, net):
        ctr_scope = tf.variable_scope('ctr_mlp',
                                      partitioner=self.__dense_layer_partitioner,
                                      reuse=tf.AUTO_REUSE)
        with ctr_scope.keep_weights() if self.__bf16 else ctr_scope:
...
if __name__ == '__main__':
    ...
    train_file = os.path.join(args.data_location, 'taobao_train')
    test_file = os.path.join(args.data_location, 'taobao_test')
    ...
    if not os.path.exists(train_file) or not os.path.exists(test_file):
        raise ValueError('[ERROR] taobao_train or taobao_test does not exist '
